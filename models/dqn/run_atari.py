...
FLAGS = flags.FLAGS
flags.DEFINE_string('environment_name', 'Seaquest', 'Atari name without NoFrameskip and version, like Breakout, Pong, Seaquest.')
...
flags.DEFINE_integer('batch_size', 128, 'Sample batch size when updating the neural network.')
...
flags.DEFINE_float('exploration_epsilon_end_value', 0.00001, 'End (decayed) value of the exploration rate in e-greedy policy.')
flags.DEFINE_float(
...
flags.DEFINE_integer('learn_interval', 10, 'The frequency (measured in agent steps) to update parameters.')
flags.DEFINE_integer(
...
def main(argv):
    ...
    logging.info(torch.from_numpy(obs[None, ...]).shape)
    ...
    # Setup checkpoint.
    checkpoint = PytorchCheckpointManager(environment_name=FLAGS.environment_name, agent_name='DQN', save_dir=FLAGS.checkpoint_dir)
