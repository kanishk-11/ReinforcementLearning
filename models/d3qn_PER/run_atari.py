...
from main.networks.value_networks import DQNConvNet, DQNConvNetAA
...
def main(argv):
    ...
    network = DQNConvNetAA(state_dim=state_dim, action_dimension=action_dim)
    ...
    train_agent.agent_name= "PER-DDDQN"
    ...
    eval_agent = EpsilonGreedyActor(
        ...
        random_state=random_state,
        device=runtime_device,
        name='PER-DDDQN-greedy',
    )

    # Setup checkpoint.
    checkpoint = PytorchCheckpointManager(
        environment_name=FLAGS.environment_name, agent_name='PER-DDDQN', save_dir=FLAGS.checkpoint_dir
    )
