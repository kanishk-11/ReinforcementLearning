...
class DDQN(Agent):
    ...
    def _calculate_loss(self, transitions: Transition) -> torch.Tensor:
        ...
        s_tm1 = torch.from_numpy(transitions.state_t_minus_1).to(device=self._device, dtype=torch.float32)  # [batch_size, state_shape]
        a_tm1 = torch.from_numpy(transitions.action_t_minus_1).to(device=self._device, dtype=torch.int64)  # [batch_size]
        r_t = torch.from_numpy(transitions.reward_t).to(device=self._device, dtype=torch.float32)  # [batch_size]
        s_t = torch.from_numpy(transitions.state_t).to(device=self._device, dtype=torch.float32)  # [batch_size, state_shape]
        ...
        q_tm1 = self._main_network(s_tm1).q_values  

        with torch.no_grad():
            q_t_selector = self._main_network(s_t).q_values
            target_q_t = self._target_network(s_t).q_values

        loss = loss_lib.double_q_learning_loss(q_tm1, a_tm1, r_t, discount_t, target_q_t, q_t_selector).loss
